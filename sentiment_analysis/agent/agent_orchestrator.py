...
from sentiment_analysis.agent.agent_factory import create_analyze_func
...
def run_agents(
    ...
    """为每段对话的每个评论节点运行 Agent 分析。

    参数:
        conversations: 对话列表
        profile_manager: 用户画像管理器（可选）
        use_llm: True=LLM Agent 优先，False=纯规则 Agent

    返回:
        agent_results: 每个对话的 Agent 结果列表
        sentiment_matrix_list: 每个对话的情绪矩阵，shape (num_nodes, 4)
    """
    analyze_comment = create_analyze_func(use_llm)
