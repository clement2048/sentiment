...
用法:
    python -m sentiment_analysis.main --mode train --input dataset/result/parsed_28.jsonl
    python -m sentiment_analysis.main --mode analyze --input dataset/result/parsed_28.jsonl --post-id 317353268392961
"""
...
from sentiment_analysis.agent.agent_orchestrator import run_agents
from sentiment_analysis.agent.user_profile import UserProfileManager
...
from sentiment_analysis.data.graph_builder import build_graphs
from sentiment_analysis.data.loader import load_conversations
from sentiment_analysis.data.preprocessor import preprocess
from sentiment_analysis.features.feature_pipeline import FeaturePipeline
from sentiment_analysis.gnn.dataset import split_graphs
from sentiment_analysis.gnn.model import SentimentGCN
from sentiment_analysis.gnn.predictor import SentimentPredictor
from sentiment_analysis.gnn.trainer import train_model
...
def cmd_train(jsonl_path: Path, use_llm: bool = False) -> dict:
    """训练模式：完整管线。
    ...
    _, sentiment_list = run_agents(conversations, profile_manager, use_llm=use_llm)
...
def cmd_analyze(jsonl_path: Path, post_id: str | None = None, use_llm: bool = False) -> None:
    ...
    if not checkpoint_path.exists():
        logger.error("模型文件不存在: %s\n请先运行训练: python -m sentiment_analysis.main --mode train", checkpoint_path)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python -m sentiment_analysis.main --mode train
  python -m sentiment_analysis.main --mode train --input dataset/result/parsed_28.jsonl
  python -m sentiment_analysis.main --mode analyze --post-id 317353268392961
        """,
    )
    parser.add_argument(
        "--mode",
